...
class GRNet(torch.nn.Module):
    ...
    def __init__(self, npoint=1024, up_ratio=2, use_normal=False, use_bn=False, use_res=False, gridding_res=64):
        ...
        self.fc11 = torch.nn.Sequential(
            torch.nn.Linear(3840, 3840),
            torch.nn.ReLU()
        )
        self.fc12 = torch.nn.Sequential(
            torch.nn.Linear(3840, 960),
            torch.nn.ReLU()
        )
    ...
    def forward(self, points, npoint=None):
        ...
        sparse_cloud = self.point_sampling(sparse_cloud, partial_cloud) + partial_cloud # ([batch_size, 4096, 3])
        ...
        # MLP
        point_features = self.fc11(point_features)  # ([batch_size, 4096, 3840]) 
